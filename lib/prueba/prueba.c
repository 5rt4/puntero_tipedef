#include "ciclismo.h"
volatille int t_inicial=0,t_final=0,vuelta_terminada=0;
...
NVIC_EneableIRQ(EXTI1_IRQn);
NVIC_SetPriority(EXTI1_IRQn,1);
NVIC_EneableIRQ(EXTI2_IRQn);
...
for(int i=0;i<4;i++){
    ...
    s->tiempoH_puerto -> CRH &=~ (0xF<<((s->th_pines[i]%8)*4));
    s->tiempoH_puerto -> CRH &=~ (0xF<<((s->t_h_pines[i]&8)*4));
...
int tiempo(freddy_t* s){
    int t_low= s->tiempo_L_puerto -> IDR & (0x4);
    int t_high= s->tiempoH_puerto ->IDR & (0x4<<12);
    return (t_low|(t_high<<4));
}


int carrera(freddy_t* s){
    while(vueltas<5){
        if(vuelta_terminada){
int v[5];
